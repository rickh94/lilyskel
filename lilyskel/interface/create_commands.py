...
def add_prompt_commands_from_list(
    list_of_names: list,
    help_prefix: str,
...
def create_prompt_command(
    name: str, attribute: str, help_text: str, *, get_completer: FunctionType = None
):
    def _item_prompt():
        ...
        if get_completer:
            ...
            completer = ctx.obj.completers.get(
                name, generate_completer(name, ctx.obj, get_completer)
            )
        old_value = rgetattr(ctx.obj, attribute) or ""
        new_value = prompt(
            HTML(f"<b>Enter {name}:</b> "), default=old_value, completer=completer
        )
