...
def composer(ctx, name):
    ...
    try:
        ...
        newcomp.mutopianame = prompt("Enter correct Mutopia Name: ")
    if not confirm(
        f"Assumed short name is {newcomp.get_short_name()}" " Correct? [Y/n] "
    ):
...
def db_instrument_prompt(instruments, ins_list, db_):
    ...
    while True:
        new_ins_name = prompt(
            "Enter an instrument, blank to finish: ", completer=ins_completer
        )
        if len(new_ins_name) == 0:
            break
        ins_number = (
            prompt(
                "Enter associated number (e.g. Violin 2) or [enter] for none: ",
...
def delete(ctx, table, field, search_term):
    ...
    while True:
        choice = prompt(
            "Type number to delete or [enter] to finish: ",
            validator=IndexValidator(len(items)),
        )
        if not choice:
            return
