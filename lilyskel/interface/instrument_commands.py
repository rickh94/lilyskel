...
from lilyskel.interface.common import create_instrument, save_non_interactive, ask_to_save, reorder_instruments
...
def delete(ctx):
    piece = ctx.obj.piece
    delete_index = radiolist_dialog(title="Delete Instrument",
                                    values=[(index, instrument.part_name) for index, instrument in
                                            enumerate(piece.instruments)])
...
_instrument_repl = sub_repl.create(instruments, {'message': 'lilyskel:edit:instruments> '},
