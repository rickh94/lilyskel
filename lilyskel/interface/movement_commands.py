...
from lilyskel.interface.custom_validators_completers import InsensitiveCompleter, NoteValidator, ModeValidator
...
def create(ctx):
    ...
    piece.movements.append(movement_prompt(tempo_completer, mode_completer, new_mov_num))
...
def update(ctx):
    ...
    piece = ctx.obj.piece
    mov_idx = radiolist_dialog(title="Update Movement",
                               text="Select a movement to update",
                               values=[(index, str(movement)) for index, movement in enumerate(piece.movements)])
    if mov_idx is None:
    ...
    tempo_completer = ctx.obj.completers.get('tempo', generate_completer('tempo', ctx.obj, _get_tempo_completer))
    mode_completer = ctx.obj.completers.get('mode', generate_completer('mode', ctx.obj, _get_mode_completer))
    new_mov = movement_prompt(tempo_completer,
                              mode_completer,
...
def delete(ctx):
    piece = ctx.obj.piece
    mov_idx = radiolist_dialog(title="Delete Movement",
...
def movement_prompt(tempo_completer: InsensitiveCompleter, mode_completer: WordCompleter, number: int,
                    existing_tempo: str = '', existing_time: str = '',
                    existing_key: info.KeySignature = info.KeySignature('', '')):
    new_mov_tempo = prompt("Enter tempo (optional): ", completer=tempo_completer, default=existing_tempo)
    new_mov_time = prompt("Enter time signature (optional): ", default=existing_time)
    new_mov_key_note = prompt("Enter key note: ", validator=NoteValidator(), default=existing_key.note)
    ...
    return info.Movement(num=number, tempo=new_mov_tempo, time=new_mov_time,
                         key=new_mov_key)
...
_movements_repl = sub_repl.create(movements, {'message': 'lilyskel:edit:movements> '},
