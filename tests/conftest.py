...
from lilyskel import info, lynames
...
@pytest.fixture()
def livedb(livedb_file):
    """A live database with data."""
    return TinyDB(livedb_file)

@pytest.fixture()
def livedb_file(tmpdir_factory):
    livedb_dir = Path(tmpdir_factory.mktemp("livedb"))
    test_db = basedir / "tests" / "test_db.json"
    shutil.copy2(test_db, livedb_dir)
    return livedb_dir / "test_db.json"
...
@pytest.fixture
def good_beethoven_file(good_beethoven, tmp_path):
    beethoven = tmp_path / "beethoven.yml"
    with beethoven.open("w") as bf:
...
@pytest.fixture
def lily_version():
    ly_vers = subprocess.run(["lilypond", "--version"], stdout=subprocess.PIPE)
    matchvers = re.search(
...
def test_ins2():
    ...
    return lynames.Instrument.numbered_name(
        "violoncello", 2, abbr="Vc.", clef="bass", midi="violoncello", family="strings"
    )
...
def bach():
    ...
    return info.Composer(
        name="Johann Sebastian Bach", mutopianame="BachJS", shortname="J.S. Bach"
    )
...
def headers1(bach):
    """A headers instance."""
    return info.Headers(title="Test Piece", composer=bach, tagline="mytagline")


@pytest.fixture
def debussy():
    """A composer instance with shortname."""
    return info.Composer(name="Claude Debussy", shortname="Claude Debussy")
...
def jinja_env():
    ...
    return Environment(loader=PackageLoader("lilyskel", "templates"))


@pytest.fixture
def mov_one_all():
    """Populated first movement."""
    return info.Movement(num=1, tempo="Allegro", time="4/4", key=("a", "major"))
...
def piece1(headers1, three_movs, instrument_list1):
    ...
    return info.Piece.init_version(
        language="english",
        headers=headers1,
