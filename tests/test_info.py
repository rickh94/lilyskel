...
from lilyskel import db_interface, exceptions, info, lynames
...
def brahms():
    ...
    return info.Composer(name="Johannes Brahms", mutopianame="BrahmsJ")
...
class TestComposer:
    ...
    def test_get_short_name(self, bach, beethoven, brahms, debussy, anon):
        ...
        assert bach.get_short_name() == "J.S. Bach", "Should get the attribute."
        assert (
            beethoven.get_short_name() == "L.v. Beethoven"
        ), "Should generate a short name properly"
        assert (
            brahms.get_short_name() == "J. Brahms"
        ), "Should generate a short name properly"
        assert (
            debussy.get_short_name() == "Claude Debussy"
        ), "Manually setting the attribute should override the method."
        assert (
            anon.get_short_name() == "Anonymous"
        ), "Single name composers should work properly"

    def test_mutopia_name(self, bach, beethoven, brahms, debussy, anon, fakecomposer):
        """Test getting the mutopia_ name."""
        assert bach.get_mutopia_name() == "BachJS", "Should return manual attribute."
        assert (
            brahms.get_mutopia_name(guess=True) == "BrahmsJ"
        ), "Should return manual attribute, even when told to guess."
        assert (
            debussy.get_mutopia_name(guess=True) == "DebussyC"
        ...
        assert (
            beethoven.get_mutopia_name(guess=True) == "BeethovenLv"
        ), "Should find one when missing."
        assert (
            beethoven.get_mutopia_name(guess=False) == "BeethovenLv"
    ...
    def test_load_from_db(self, bach, livedb):
        ...
        assert (
            info.Composer.load_from_db(name="Johann Sebastian Bach", db=livedb) == bach
    ...
    def test_add_to_db(self, beethoven, debussy, mockdb):
        ...
        assert db_interface.explore_table(comptable, search=("name", "Claude Debussy"))
...
def mutopiaheader2(random_ens):
    ...
    return info.MutopiaHeaders(
        instrument_list=random_ens,
...
def test_convert_instruments(mutopiaheader2, test_ins, test_ins2, test_ins3, test_ins4):
...
def test_validate_instruments(mutopiaheader1):
    ...
    with pytest.raises(TypeError, match="instruments.*list.*"):
        mutopiaheader1.validate_instruments(
            "instrument_list", ["not", "an", "instrument"]
        )
...
class TestHeaders:
    ...
    def test_add_mutopia_headers(self, headers1, mutopiaheader1):
        ...
        assert (
            "Violin" in headers1.mutopiaheaders.instruments
        ), "instruments should have been loaded."
        assert (
            "Creative" in headers1.copyright
    ...
    def test_headers_load(self, headers1, mutopiaheader1, headers2):
        ...
        instruments1 = [
            info.Instrument.load(ins)
            for ins in newheadersdict["mutopiaheaders"]["instrument_list"]
        ]
        ...
        assert (
            newheaders_loaded.mutopiaheaders.source == newheaders.mutopiaheaders.source
        )
        assert (
            newheaders_loaded.mutopiaheaders.license
            == newheaders.mutopiaheaders.license
        )
        headers2dict = headers2.dump()
        instruments2 = [
            info.Instrument.load(ins)
            for ins in headers2dict["mutopiaheaders"]["instrument_list"]
        ]
        ...
        assert headers2_loaded.mutopiaheaders.source == headers2.mutopiaheaders.source
        assert headers2_loaded.mutopiaheaders.license == headers2.mutopiaheaders.license
...
class TestPiece:
    ...
    def test_init_version(self, headers1, instrument_list1):
        ...
        test = info.Piece.init_version(
            headers=headers1, language="english", instruments=instrument_list1
        )
    ...
    def test_piece_load(self, piece1, piece2):
        ...
        assert (
            new_piece2.headers.mutopiaheaders.maintainer
