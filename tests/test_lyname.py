...
from lilyskel import exceptions, lynames
...
class TestRomanNumeral(object):
    ...
    def test_errors(self):
        ...
        with pytest.raises(TypeError, match=r"num.*int"):
            lynames._roman_numeral("hi")

        with pytest.raises(ValueError, match=r".*1 and 89"):
...
class TestLyName:
    ...
    def test_mov_file_name(self, testlyname1, testlyname2):
        ...
        # test mov_file_name
        assert testlyname1.mov_file_name(1) == "global_1.ily", "should append number"
        assert testlyname1.mov_file_name(2) == "global_2.ily", "should append number"
        assert testlyname2.mov_file_name(1) == "test2_1.ily", "should append number"

    def test_var_name(self, testlyname1, testlyname2):
        """Tests getting the variable name."""
        assert (
            testlyname1.var_name(2) == "\\global_second_mov"
        ), "should append number words"
        assert (
            testlyname1.var_name(31) == "\\global_thirty_first_mov"
        ), "should append number words"
        assert (
            testlyname2.var_name(2) == "\\test_two_second_mov"
        ), "should append number words"
        assert (
            testlyname2.var_name(2, slash=False) == "test_two_second_mov"
    ...
    def test_movement_exceptions(self, testlyname1, testlyname2):
        ...
        with pytest.raises(ValueError, match=".*'form'.*'word'"):
    ...
    def test_dir_name(self, testlyname1, testlyname2):
        ...
        assert testlyname1.dir_name() == "global", "should return without set number"
        assert testlyname2.dir_name() == "test2", "should append the number if present."
    ...
    def test_part_file_name(self, testlyname1, testlyname2):
        ...
        """
        assert (
            testlyname1.part_file_name() == "global.ly"
        ), "should return without prefix."
        assert (
            testlyname1.part_file_name(prefix="abc") == "abc_global.ly"
        ), "Should add prefix and underscore."
        assert (
            testlyname1.part_file_name(prefix=123) == "123_global.ly"
        ), "should handle non string types."
        assert testlyname2.part_file_name() == "test2.ly", "should work with numbers."
        assert (
            testlyname2.part_file_name(prefix="O12") == "O12_test2.ly"
...
class TestInstrument:
    ...
    def test_part_name(self, test_ins, test_ins2, test_ins3):
        ...
        assert (
            test_ins3.part_name(key=True) == "Clarinet in Bb"
        ), "should have key in name"

    def test_load_from_db(self, mockdb, livedb):
        """Tests loading an object from tinydb."""
        assert lynames.Instrument.load_from_db("flute", mockdb) == lynames.Instrument(
            "flute",
            abbr="Fl.",
            clef="treble",
            transposition=None,
        ...
        assert lynames.Instrument.load_from_db(
            "flute", mockdb, number=2
        ...
        assert lynames.Instrument.load_from_db("violin", livedb) == lynames.Instrument(
            "violin",
            abbr="Vln.",
            clef="treble",
            transposition=None,
        ...
        with pytest.raises(exceptions.DataNotFoundError, match=".*not in.*table"):
    ...
    def test_get_mutopia_name(self, test_ins, test_ins2, test_ins3, test_ins4):
        ...
        assert test_ins3.get_mutopia_name() == "Clarinet", "Should match closest."
        assert (
            test_ins4.get_mutopia_name() == "Oboe_"
...
class TestEnsemble:
    ...
    def test_add_instrument(self, livedb, violin1):
        ...
        assert violin1 in test_ens.instruments, (
            "Should have loaded 'violin' " "object from the database"
        )
        test_ens.add_instrument(
            ins_name="glockenSPiel",
            number=1,
            abbr="Gl.",
            clef="treble",
            keyboard=False,
        ...
        glock = lynames.Instrument.numbered_name(
            name="glockenspiel",
            number=1,
            abbr="Gl.",
            clef="treble",
        ...
        assert glock in test_ens.instruments, (
            "Should have created "
    ...
    def test_load_from_db(
        self, livedb, monkeypatch, violin1, violin2, viola, violoncello
    ):
        ...
        monkeypatch.setattr(
            "lilyskel.lynames.Instrument.load_from_db", raises_data_not_found
        )

        with pytest.raises(
            exceptions.MissingInstrumentError,
            match=".*instrument.*string_quartet.*database" ".*thing not found here.*",
        ):
    ...
    def test_add_to_db(self, livedb):
        ...
        wind_quartet.add_instrument(
            "flute", abbr="Fl.", midi="flute", family="woodwinds"
        )
        wind_quartet.add_instrument(
            "clarinet_in_bb",
            abbr="Cl.",
        ...
        wind_quartet.add_instrument("Oboe", abbr="Ob.", midi="oboe", family="woodwinds")
        wind_quartet.add_instrument(
            "Bassoon", abbr="Bsn.", midi="bassoon", family="woodwinds"
        )
        ...
        for ins in instrument_list:
            assert ins in wq["instruments"], (
                "All instrument names should be " "represented in the db."
            )

        # all instruments should have been added to the database.
        assert instruments.search(
            Search.name == "flute"
        ...
        assert instruments.search(
            Search.name == "clarinet_in_bb"
        ), "There should be a clarinet_in_bb in the database now."
        assert instruments.search(
            Search.name == "bassoon"
        ), "There should be a bassoon in the database now."

        wind_quartet_loaded = lynames.Ensemble.load_from_db("wind quartet", livedb)

        assert wind_quartet_loaded == wind_quartet, (
