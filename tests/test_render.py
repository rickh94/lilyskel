...
from lilyskel import lynames, render
...
def test_make_instrument(
    test_ins, test_ins2, test_ins3, piece1, piece2, tmpdir, lyglobal
):
    ...
    assert os.path.exists(Path(tmpdir, "clarinet_in_bb", "clarinet_in_bb_1.ily"))
    assert os.path.exists(Path(tmpdir, "clarinet_in_bb", "clarinet_in_bb_2.ily"))
    assert os.path.exists(Path(tmpdir, "clarinet_in_bb", "clarinet_in_bb_3.ily"))
...
def test_render_includes(piece1, tmpdir):
    ...
    render.render_includes(includepaths=includes, piece=piece1, location=tmpdir)
