...
def test_defs(tmpdir, jinja_env, piece1, piece2):
    ...
    defs_template = jinja_env.get_template("defs.ily")
    with Path(tmpdir, "defs_test1.ily").open("w") as defs1:
        defs1.write(defs_template.render(piece=piece1))
    with Path(tmpdir, "defs_test2.ily").open("w") as defs2:
        defs2.write(defs_template.render(piece=piece2))

    with Path(tmpdir, "defs_test1.ily").open("r") as defs1:
        test1 = defs1.read()
    with Path(tmpdir, "defs_test2.ily").open("r") as defs2:
        test2 = defs2.read()

    assert "Johann Sebastian Bach" in test1, "Composer name should be in the file"
...
def test_ins_part(tmpdir, jinja_env, test_ins, test_ins3, piece1, piece2, piano):
    """Tests rendering an instrument part."""
    instemplate = jinja_env.get_template("ins_part.ly")
    lyglobals = lynames.LyName(name="global")
    flags1 = {"key_in_partname": False, "compress_full_bar_rests": True}
    flags2 = {"key_in_partname": True, "compress_full_bar_rests": False}
    render1 = instemplate.render(
        piece=piece1,
        instrument=test_ins,
        lyglobal=lyglobals,
        flags=flags1,
    ...
    render2 = instemplate.render(
        piece=piece2,
        instrument=test_ins3,
        lyglobal=lyglobals,
        flags=flags2,
        filename="inspart_test2.ly",
    )
    render3 = instemplate.render(
        piece=piece1,
        instrument=piano,
        lyglobal=lyglobals,
        flags=flags1,
...
def test_global(tmpdir, jinja_env, global_ins, piece1):
    globalpath = Path(tmpdir, "global")
    dirpath = Path(tmpdir, global_ins.dir_name())
    os.makedirs(dirpath)
    template = jinja_env.get_template("global.ily")
    for mov in piece1.movements:
        render = template.render(piece=piece1, lyglobal=global_ins, movement=mov)
        with Path(dirpath, global_ins.mov_file_name(mov.num)).open("w") as out:
            out.write(render)
...
def test_notes(
    tmpdir, jinja_env, test_ins, test_ins2, three_movs, piece1, global_ins, piano
):
    ...
    for ins in [test_ins, test_ins2, piano]:
        ...
        os.makedirs(dirpath)
        for mov in piece1.movements:
            render = template.render(piece=piece1, instrument=ins, movement=mov)
...
def test_includes(tmpdir, jinja_env, piece1):
    ...
    render = template.render(
        piece=piece1, extra_includes=extra_includes, includepaths=includepaths
    )
...
def test_score(piece1, jinja_env, instrument_list1, piano, tmpdir):
    ...
    score_path = Path(tmpdir, "score.ly")
    with score_path.open("w") as score_file:
        score_file.write(render)

    with score_path.open("r") as score_file:
        score_render = score_file.read()

    assert "Test Piece" in score_render
    assert "\\version" in score_render
    assert "\\violin_one_first_mov" in score_render
    assert "\\global_first_mov" in score_render
    assert "\\violoncello_two_third_mov" in score_render
    assert "\\piano_first_mov_LH" in score_render
    assert 'shortInstrumentName = "Cl."' in score_render
    assert 'instrumentName = "Violin I"' in score_render
